...
async def login(request: Request, response: Response, user: UserBody, csrf_protect: CsrfProtect = Depends()): #クライアントからのemail とpasswordを受け取る
    csrf_token = csrf_protect.get_csrf_from_headers(request.headers)
    csrf_protect.validate_csrf(csrf_token)
    user = jsonable_encoder(user) #dictに
    token = await db_login(user) #dict->str
    response.set_cookie(
...
def logout(request: Request, response: Response, csrf_protect: CsrfProtect = Depends()):
    csrf_token = csrf_protect.get_csrf_from_headers(request._headers)
    csrf_protect.validate_csrf(csrf_token)
