...
async def db_get_todos() -> list:
    todos=[]
    for todo in await collection_todo.find().to_list(length=100):# motorの機能(メソッドfind(), to_list)　；length:取得する最大
...
async def db_update_todo(id: str, data: dict) -> Union[dict, bool]:
    ...
    if todo:
        ...
        if (updated_todo.modified_count > 0): #更新に成功したら
...
async def db_signup(data: dict) -> dict:
    ...
    user = await collection_user.insert_one({"email": email, "password": auth.generate_hashed_pw(password)}) #insert_oneの返り値は、InsertOneResult(インスタンス)
